<template>
  <div>
    <!-- 信息展示 -->
    <el-descriptions title="达成的协议">
      <el-descriptions-item label="谈判结果">
        <el-tag>{{ }}</el-tag>
      </el-descriptions-item>
      <el-descriptions-item label="最后一次出价">{{ agreementValue }}</el-descriptions-item>
      <el-descriptions-item label="最后一次出价者">{{ }}</el-descriptions-item>
      <el-descriptions-item label="我方代理">{{ }}</el-descriptions-item>
      <el-descriptions-item label="对方代理">{{ }}</el-descriptions-item>
      <el-descriptions-item label="谈判步数">{{ }}</el-descriptions-item>
      <el-descriptions-item label="谈判时间">{{ }}</el-descriptions-item>
      <el-descriptions-item label="我方收益">{{ }}</el-descriptions-item>
      <el-descriptions-item label="对方收益">{{ }}</el-descriptions-item>
      <el-descriptions-item label="帕累托距离">{{ }}</el-descriptions-item>
      <el-descriptions-item label="纳什均衡距离">{{ }}</el-descriptions-item>
    </el-descriptions>
    <!-- 在页面顶部展示协议 -->
    <!-- CSV 数据表格 -->
...
const agreementValue = ref({});  // 存储协议内容 { interest -> issue }
...
async function transposeData() {
  ...
  csvData.value.forEach(row => {
    ...
    const result = {};
    for (let r in row) {
      if (r === 'self_agent' || r === 'opp_agent'){
        if (row[r] === "AgentA") {
          row[r] = roles_data_content.value[0];
        } else if (row[r] === "AgentB") {
          row[r] = roles_data_content.value[1];
        }
      }
      result[r] = row[r];
    }
